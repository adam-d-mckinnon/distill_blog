...
# 1. Create Time Series Data
The below code stub generates fictitious data for our example. Please note that you can replicate this example, however, the data generated will be different, which in turn may lead to different outcomes (i.e., both visualisations, summary statistics and benefit quantified).



```{r ca_setup, echo=TRUE}
# load libraries
...
The graphical visualisation of the model results is provided below. The top panel of the visualisation shows a blue confidence interval, in which we would expect our time series data to remain in the absence of the campaign. The second panel displays the difference between situation normal (i.e., no campaign), which is depicted by the the zero value line in the graph, and what happened in reality. In our example we see the pointwise values occur both above and below the zero line. The third panel provides a perspective on the cumulative benefit of the campaign. The visualisation adds the pointwise contributions from the second panel, showing the benefit was minimal and only realised at the end of the post intervention period. None of these visualisation panels inspire confidence in our campaign!



```{r uk_only_model, include=TRUE}
...
# get a graphical summary of the impact
graphics::plot(impact_uk_only) %>% 
    plotly::plotly_build()


# get a summary &/or report of the model
base::summary(impact_uk_only) # summary version

# plain text walk through of result - not run
# base::summary(impact_uk_only, "report") 
...
```{r all_countries_model, include=TRUE}
...
# get a summary &/or report of the model
base::summary(impact_all_data) # summary version
# plain text walkthrough of result - not run
# base::summary(impact_all_data, "report") 
...
Based on the period we analysed (i.e., 1/10/2018 - 31/12/2018) our second model indicates that under normal circumstances we would have expected 100 employees on average to take leave each day, while the campaign seems to have catalysed a daily average of 107 employees. The cumulative benefit of this campaign is an additional 437 UK employees taking leave during the post campaign period (i.e., subtracting our cumulative prediction (6599) from our cumulative actual (7036)).

If the average daily cost of an UK employee is $300 GBP we could simply determine the financial benefit of the campaign by multiplying 437 by our average daily cost of an employee (i.e., $300 GBP). The gross estimated financial benefit of the campaign would be $131,100 GBP, which can be narrowed to a net financial benefit of $101,100 GBP, after subtracting our campaign costs of $30,000 GBP. Not bad HR!
...
1. measure the ROI (Return-on-Investment) of such behavioural campaigns (e.g. marketing campaign), especially when we are not sure if the campaign was the only single source of the impact; and
